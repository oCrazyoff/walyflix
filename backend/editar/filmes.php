...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $sql = "UPDATE filmes SET titulo = ?, descricao = ?, ano = ?, categoria_id = ?, imagem_url = ?, link_filme = ? WHERE id = ?";
        $stmt = $conexao->prepare($sql);
        $stmt->bind_param("ssiissi", $titulo, $descricao, $ano, $categoria_id, $imagem_url, $link_filme, $id);
